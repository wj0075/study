...
(function(){
    ...
    /**
     * move:运动库
     * @param opt：{id:元素,target:目标值,callback:回调,duration:总时间，effect:运动形式}
     */
    function move(opt){
        if(!opt.id) return;//当没有指定运动的元素的时候，直接阻断程序执行；
        var tmpEffect=null;
        var ary = ["Linear", "Elastic-easeOut", "Back-easeOut", "Bounce-easeOut", "Expo-easeIn"];
        //设置默认值：把所有的默认值都统一设置到这里；
        var defaultOpt={
            effect:1,
            duration:700
        };
        for(var attr in opt){
            defaultOpt[attr]=opt[attr];//用户传了，按用户传的来，否则，按默认值来；
        }
        ...
        if(typeof defaultOpt.effect === 'number'){
            var str=ary[defaultOpt.effect%ary.length];//%思想的运动:有几种情况就%几；
