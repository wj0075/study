/**
 * Created by wj007 on 2016/11/21.
 */
var utils = (function(){
    ...
    function rnd(n,m){
        ...
        m = Number(m);
        if (isNaN(n)||isNaN(m)){
            return Math.random();//返回0-1之间的随机小数，说明参数传错了；
        }
        if (n>m){
            var tmp = m;
            m=n;
    ...
    function setCss(curEle,attr,value){
        ...
        var reg = /(width|height|top|bottom|right|left|((margin|padding)(left|top|right|bottom)?))/gi;
        //升级1：处理单位
        if(reg.test(attr)){
    ...
    function getChildren(curEle,tagName){
        var childs = curEle.childNodes;//获取所有子节点
        var ary = [];
